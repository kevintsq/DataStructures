...
int main() {
    ...
    scanf("%d", &n);
    //clock_t t1 = clock();
    register int i, j, index, cnt = 0, union_sum1 = 0, union_sum2 = 0, sum1 = 0, sum2 = 0;
    ...
    while ((c = fgetc(fp)) != EOF) {
        if ('a' <= c && c <= 'z')
            ...
            word[i] = '\0';
            for (p = dict, j = 0; word[j] != '\0'; j++) {  // p首先在根节点
                index = word[j] - 'a';  // 依次取出word中的字母
                if (p->next[index] == NULL) {  // 若该位置的字母没有子树，为其分配子树
                    p->next[index] = (node *) calloc(1, sizeof(node));
